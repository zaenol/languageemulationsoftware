...
public class MakeWordList {
	...
	public MakeWordList(String fileName) throws IOException {
		...
		File file = new File("."+fullPath+fileName.replace(".csv", "_adVerbList.csv"));
		...
		BufferedReader buffRead = new BufferedReader(new InputStreamReader(in));
	      ...
	      while ((line = buffRead.readLine()) != null){
	    	...
	    	if(!word.contains("\'")){
	    		...
	    		if(pword.pos_isAdVerb()){
	    			if(!seenWords.contains(token.getValue())){
	    				output.write(token.getValue());
	    				output.newLine();
	    				seenWords.add(token.getValue());
	    			}
	    		}
	...
	public static void main(String[] args) {
		...
		try {
			MakeWordList fcsvf = new MakeWordList(fileName);
		} catch (IOException e) {
